...
@section('heading')
{{ Lang::get('common.Manage') }} {{ Lang::get('admin/user/startup.Heading') }}
@endsection
        ...
        <div class="panel panel-primary">
            <div class="panel-heading">
                <div class="panel-title">
                    <h4>{{ Lang::get('admin/user/startup.startup_list') }}</h4>
                </div>
            </div>
            <!-- Nav tabs -->
            ...
            <div class="tab-content">
                <div class="tab-pane fade in active" id="tab1">
                            ...
                            <div class="table-responsive">
                                <table id="dataTableExample2" class="table table-bordered table-striped table-hover dataTableExample2">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>{{ Lang::get('admin/user/startup.name') }}</th>
                                            <th>{{ Lang::get('admin/user/startup.email') }}</th>
                                            <th>{{ Lang::get('admin/user/startup.mobile') }}</th>
                                            <th>{{ Lang::get('common.Action') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @foreach($startup_data as $value)
                                        <?php $id = $value['id']; ?>
                                        <tr>
                                            <th scope="row">{!! $i !!}</th>
                                            ...
                                            <td><a href="{{url("admin/startup/$id/edit")}}"><button type="button" class="btn btn-primary btn-circle m-b-5"  title="{{ Lang::get('common.Edit') }}" ><i class="glyphicon glyphicon-edit"></i></button></a></td>
                                        </tr>
                                        <?php $i++; ?>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="panel-footer">
                        {{ $startup->appends($_GET)->links('vendor/pagination/default') }}
                        <div class="pageTotal">{{ Lang::get('common.Total') }} {{ $startup->total() }} {{ Lang::get('common.Found') }}</div>
                    </div>
                </div>
                <!-- / tab 1 -->
